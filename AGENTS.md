...
- Windows / PowerShell `git commit` messages: PowerShell has no heredoc, so the `git commit -m "$(cat <<'EOF' … EOF)"` pattern fails with a `MissingFileSpecification` parser error. For any multi-line or non-ASCII commit body, write the message to a UTF-8 temp file under `tools-tmp/` and use `git commit -F tools-tmp/<file>.txt`, then delete the temp file. The same applies to `gh pr create --body-file` and `gh release create --notes-file`.
