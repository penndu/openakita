...
def test_modes_count_and_endpoints() -> None:
    assert len(MODES) == 4
    expected = {"photo_speak", "video_relip", "video_reface", "avatar_compose"}
    assert {m.id for m in MODES} == expected
    for m in MODES:
        assert m.dashscope_endpoint  # required for pipeline dispatch
...
def test_price_table_frozen_keys() -> None:
    ...
    assert PRICE_TABLE["wan2.2-s2v"]["480P_per_sec"] == 0.10
    assert PRICE_TABLE["wan2.2-s2v"]["720P_per_sec"] == 0.20
...
def test_estimate_cost_photo_speak_480p_5s() -> None:
    ...
    # 0.004 (detect) + 0.10*5 (s2v) + 0.20*(120/10000) ≈ 0.004 + 0.50 + 0.0024
    assert p["currency"] == "CNY"
    assert p["total"] == pytest.approx(0.51, abs=0.005)
