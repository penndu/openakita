...
Mitigates issue #479 ("uploaded image not visible in UI"): plugin POST
``/upload`` handlers historically returned an absolute *server-side* path,
which the browser cannot fetch.  This helper registers a tightly scoped GET
endpoint (default: ``/uploads/{rel_path:path}``) on the caller's
``APIRouter`` so the UI can render a previously uploaded asset back to the
user with a normal ``<img src="/api/plugins/<id>/uploads/<file>">`` element.
...
- ``fastapi`` is imported only inside :func:`add_upload_preview_route` so the
  SDK keeps importing cheaply for plugins that do not need HTTP routes.
- Pure helper, no global state.  Two plugins can register the same route on
  different routers with different ``base_dir`` values without collision.
"""
...
def add_upload_preview_route(
    ...
    *,
    ...
    Args:
        router: A FastAPI ``APIRouter`` (imported lazily here so the SDK does
            not pull fastapi for plugins that do not register routes).
